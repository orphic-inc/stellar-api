...
- **Asset store Phase 2 — upload, visibility, quota, and orphan sweep** ([ADR-0026](docs/adr/0026-static-asset-storage.md), #342) — the substrate from #290 gets the three pieces its Decision deferred, plus the quota that gates them. `POST /api/asset?kind=…` stores one uploaded binary (raw body, not multipart — `validateAsset` identifies it by magic bytes, so the filename multipart carries is discardable) through the new `uploadAsset` seam, restricted to `ThemeImage`/`Avatar` since shipping a font is the redistribution question #343 owns. An `Asset.visibility` column splits delivery two ways: `Public` fixtures stay unauthenticated as shipped, while `Members` uploads require auth on the serve route and cache `private`, matching the `/css` route on an invite-only instance — no owner-private tier, because nothing produces one and an unexercised authorization branch rots. A daily `assetSweepJob` collects member-owned assets that no stylesheet source or avatar references and that are past a 24h grace window (a scan, not a reference table: content-addressing means one asset can have many referrers, and a scan cannot drift the way a write-path counter can); site-owned assets are never swept. `UserRank.assetByteLimit` (0 = unlimited) caps the sum of a member's stored bytes, and re-uploading bytes already owned is not charged.

  This also makes an `img-src 'self'` CSP reachable, which is the proper fix path for the #361 avatar-hotlink disclosure: `profile.avatar` now accepts a self-hosted `/api/asset/<sha256>` path. Tightening `avatar` to self-hosted-only stays #361's call — remote URLs still validate.
