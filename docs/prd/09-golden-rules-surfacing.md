...
| `${irc_guide_article}`                                                                                               | Public KB | `config.site.ircGuideUrl` (`STELLAR_IRC_GUIDE_URL`, default `${public_kb}/irc`); also named by the disabled login `403` and the deactivation email (#622) |
