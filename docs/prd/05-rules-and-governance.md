...
| **InterviewRules** | recruitment/interview conduct — net-new; [spec'd, unbuilt](../governance/forum-and-staff-rules.md#interviewrules-spec--not-built): governs interviewee + interviewer asymmetrically (applicants have no account to score, so their side gates entry; interviewer breaches score as StaffRules) |
...
| **IRCRules**       | conduct on the IRC network — **rules here; the IRC feature itself is PRD-02**; [spec'd, unbuilt](../governance/forum-and-staff-rules.md#ircrules-spec--not-built): `irc.conduct` node, v1 enforcement is staff-filed (no automated korin violation feed), gated on the ADR-0015 verified nick  |
...
3. ~~**Document ForumRules/StaffRules** against the built code; spec IRCRules + InterviewRules as net-new.~~ **Shipped: [#126](https://github.com/orphic-inc/stellar-api/issues/126)** — all four sub-rulesets now live in [governance/forum-and-staff-rules.md](../governance/forum-and-staff-rules.md). ForumRules + StaffRules are documented **as built** (sub-rule list, the `ruleImpact()` weights as implemented, and an honest built-vs-stub map: `ForumSpecificRule` is a stub, the staff "+50 CRS/week-served" signal is not yet a CRS dimension). IRCRules + InterviewRules are **spec'd but unbuilt** — content and relative weight ordering decided, absolute magnitudes deliberately left to the open question below rather than fiat'd into the only weighted rules on the site. Implementation (seeders + rows) is a follow-up.
