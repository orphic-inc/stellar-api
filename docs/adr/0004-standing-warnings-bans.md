...
_(Extended 2026-09-15, by [ADR-0039](0039-invite-supply-is-class-based-accrual.md) §4 and [ADR-0043](0043-sending-an-invite-is-gated-on-the-inviter.md). "Never gates access" no longer holds for invites. At `poor` and `hammer`, standing withholds the invite handout ([#282](https://github.com/orphic-inc/stellar-api/issues/282)) and refuses sending an invite ([#637](https://github.com/orphic-inc/stellar-api/issues/637)), both through `isStandingDenied` in `modules/inviteGrant.ts`. Everywhere else this section stands: standing does not gate downloads, content or permissions, which stay with [ADR-0001](0001-granular-permission-checks.md). The member keeps the invites they hold, and both refusals lift when the warnings expire.)_
