...
**Status:** Proposed — **blocked: no substrate.** The korin `ledger` this contract reads over is being withdrawn (see ADR-0016, Superseded); `GET /ledger/integrity` therefore has nothing to attach to. The signal taxonomy and action model below stand on their own and are worth keeping, but any implementation must **first specify and justify its own substrate** rather than inherit one. Do not build against the endpoint shape in this document until that is done. The implementation epic (stellar-api#327) and its korin detector twin (korin-pink#51) were closed as substrate-withdrawn on 2026-07-23; this ADR is now the sole surviving record, and any revival must first specify and justify its own substrate — what state it needs, where it lives, and what problem observed in stellar it solves. Note that invite-lineage sybil suspicion is already realized stellar-locally by `contagion.ts` (a bounded, distance-decaying negative CRS arm), so a revival would need to justify only the signal kinds it cannot already see.
**Date:** 2026-07-15
