...
- **Leader is a superset of staff.** The invariant `leaderId ⟹ user ∈ staff ∧ user is a Consumer` is enforced in every write path, so existing staff-gated checks keep working with zero "is staff OR is leader" special-casing. The pointer is purely _additional_. **Narrowed by [ADR-0033](0033-community-membership-and-the-curator-role.md) to `leaderId ⟹ user ∈ curators`:** the `Consumer` half existed only so the old `consumer ∪ contributor` checks would pass, and #419 replaced those with a role union that reads the relation directly. `staff` is renamed `curators` in the same ADR.
