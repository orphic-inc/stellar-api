...
Today, ratio accounting lives entirely in stellar-api: `downloads.ts` accrues `contributed`/`consumed` at grant time, `ratio.ts` computes required ratio, and `ratioPolicy.ts` evaluates `OK → WATCH → DOWNLOAD_DISABLED` **at read time**. There is no real-time gate at grant time, no hot working set, and no live activity stats — and computing those per-request against Postgres is the wrong place for hot, high-churn counters.
