...
- **Avatar hotlinking.** `profileSettingsSchema.avatar` is `z.string().url()` — an arbitrary remote URL, not even https-constrained the way `externalStylesheet` was tightened by ADR-0024 §3. Every viewer of a member's page therefore already leaks IP and timing to a host that member chose, with no CSS involved. This ADR does not fix it and does not pretend to: closing the CSS door while this stands would be a boundary that defends one entrance. Tracked separately.
  ...
  **Amended 2026-08-31 ([#396](https://github.com/orphic-inc/stellar-api/issues/396)).** Both avatar write paths — and the donor `customIcon`/`secondAvatar` perks, which render on the same surfaces from the same bare `.url()` — now take https or an `/api/asset/<sha256>` path only, and avatars may be self-hosted in the store. The acceptance above is **narrowed, not lifted**: an https remote avatar still discloses IP, user agent and visit timing to a host the member chose, because §6 keeps `img-src` open. Two things this paragraph got wrong are worth recording rather than quietly fixing. It named one schema; there were two, and `userSettingsSchema.avatar` was the laxer — a bare `z.string()` with no URL check at all. And it framed the hole as avatar-shaped, which set the cost of tightening `img-src` throughout this ADR (§6, Rejected); BBCode `[img]` (`lib/bbcode/render.ts`) renders an arbitrary remote image into every forum post, so `img-src 'self'` was always a larger cut than "breaks every remote avatar" implied. The CSP endgame is re-filed on that corrected footing as [#457](https://github.com/orphic-inc/stellar-api/issues/457).
