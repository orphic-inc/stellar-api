...
One test this ADR specified is only half-buildable until slice 2 lands: the `PRIVATE` + `registrationStatus: open` case (Decision 1) cannot be expressed before the column exists. What ships is the half that can be — registration status alone opens a community — plus a structural assertion over the fragment's arms that fails if a visibility arm is ever added. The full case is owed once `announceVisibility` exists.
