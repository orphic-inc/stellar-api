...
**A correction the original draft got wrong.** The draft assumed the public firehose already delivered to IRC and this feature merely _gated_ it. It did not. Tracing the path end-to-end at the time: stellar's `publishAnnounceItem` POSTed the RSS, checked `res.ok`, and **discarded the response body**; korin's `/irc/announce` parsed the feed, rendered one line, and **returned it to the caller** (`{ success, mode, artifact }`); nothing posted that line to any channel (the irc-bridge's only `client.say` calls were `!verify` replies and NickServ registration). The last mile — korin actually posting a rendered line into a channel — had never been built, for the public case either. So this ADR was not "add a gate to working delivery"; the public delivery leg was a **prerequisite** that had to land first. It has: Decision 0 is satisfied.
...
- **korin** models no channel ACL in its API. When this ADR was written it did not deliver announces to any channel either; that half has since been built (korin ADR-006, korin-pink#70/#71 — see Decision 0). Ergo/ChanServ can enforce per-channel membership/op at the ircd layer, but korin still exposes no endpoint to gate an announcement to a private channel's members. Nick ownership is proven (ADR-0015), which is the identity substrate an ACL would stand on; the ACL itself does not exist.
...
**Two axes, kept apart.** ([ADR-0033](0033-community-membership-and-the-curator-role.md) renames the Axis-1 `Community.staff[]` relation to `curators[]`, so the two axes stop sharing a word; substitute "curator" for "community staff" throughout this decision.) Membership is an _Axis-1_ property — the per-community relations on the `Community` model. It is **not** the _Axis-2_ global capability: a site staffer (the `communities_manage` / `admin` rank permission) is not a "member" of every private community and does **not** enter the eligibility union. Site staff appears only as the permission gate on the visibility-toggle route (Decision 5); it never populates a `#c-<id>` channel. `leaderId` is a distinguished element of `staff[]`, not a separate set — ADR-0021 always folds the leader into `staff[]` (and upserts a `Consumer`), so the honest union is `consumer ∪ contributor ∪ community-staff`; `leaderId` is kept only as a **defensive, redundant** arm guarding against an ADR-0021 invariant violation, not as load-bearing membership.
...
**Five call-sites, not two.** A third grill pass (2026-07-25) found the union expressed in four places, not the two an earlier draft named — and two of them are **authorization gates**, not browse filters: `isCommunityMember` (`communities.ts:54`), serving `GET /:id`, `GET /:id/health`, `GET /:id/health/history` and `releaseBrowse.ts:25`, plus a byte-identical private duplicate at `releaseWorkbench/authority.ts:18` gating release edits. Implementing #419 turned up a **fifth**: `GET /api/artists/:id` (`communities/artist.ts`) open-codes the same union as an _id list_ (`accessibleCommunityIds`) filtering an artist's credits, and carried the identical bug. The shared fragment and the gate move to a new **`src/modules/communityAccess.ts`** (which later also holds this ADR's `communityAnnounceNicks`), and both duplicates fold into it — incidentally clearing `releaseBrowse.ts:5`, the only module→route import in `src/modules/`.

The count is a floor, not a proof: each site was found by reading, and the id-list shape of the fifth is why a name-based sweep missed it. What makes the drift bounded from here is the shared fragment, not the enumeration.

`isCommunityMember` is **renamed `hasCommunityAccess`** in the same move. ADR-0001 is not violated by the `staff` arm — that ADR governs the rank-permission map (Axis-2), while `Community.staff[]` is an Axis-1 relation, and `communities.ts` already pairs relation checks with exactly-named rank permissions in four places. But ADR-0001's deeper complaint does apply: one collapsed predicate standing in for several distinct authorizations. The sites ask different questions — read a community, read its health, list its releases, see an artist's credits from it, **edit a release in it** — so widening a predicate named "member" would grant all of them silently, one being a write gate. The rename makes the widening legible at each site.
...
2. **stellar — LANDED:** `Community.announceVisibility` field + migration (default `PUBLIC`), plumbed through the create/update bodies on the existing `communities_manage` gate and exposed on the community response. Independent and forward-safe: every existing community defaults to `PUBLIC`, so announce behaviour is unchanged until slice 4 reads the column. Nothing else reads it — the omission from `communityRoleUnion` is commented at the seam and asserted by the tests named in the Amendment above.
3. **stellar — extracted to #419, LANDED ahead of the epic:** the shared role-union fragment (`consumer ∪ contributor ∪ community-staff`, `leaderId` defensive-redundant) in a new `communityAccess.ts`, the `isCommunityMember` → `hasCommunityAccess` rename, the `releaseWorkbench/authority.ts` duplicate folded in, and the browse filter refactored to compose the fragment (`OR: [ open, roleUnion ]`) + tests. Owns both halves of the widening — browse (staff-only members appear) and the gate (staff-only members stop 403ing). No schema change and no korin dependency, which is why it ships standalone. Landed with a fifth call-site (artist credits) and an `assertCommunityAccess` load-then-gate wrapper the two module-side duplicates each open-coded; see the [Amendment](#amendment-2026-07-25--implementation-of-419).
...
5. **korin (paired issue):** the `#c-<id>` channel-ACL receiver + Ergo/ChanServ enforcement (invite/op from the projected nick set, materialized-view overwrite each projection). **Must include a channel-join story, not only the ACL:** korin ADR-006 has the bridge reject any channel it has not joined, and nothing joins it to `#c-<id>`, so the first private route would 503 and then re-push every cycle on stellar's cursor hold. Noted on korin-pink#67.
...
## Amendment (2026-07-25) — implementation of #419

Slice 3 shipped. Building it corrected two claims this ADR made from reading alone:

- **Five call-sites, not four.** `GET /api/artists/:id` (`communities/artist.ts`) open-codes the union as an _id list_ — `consumer.communities ∪ contributor.communityId ∪ every open community` — filtering which of an artist's credits a caller sees. Same bug, same fix: a staff-only member saw no credits from the communities they staff. It now composes `communityRoleUnion`, three queries collapsing to one. The id-list shape is why the grill pass's name-based search missed it, which is the honest reason to treat "five" as a floor rather than a proof — the shared fragment is what bounds the drift, not the count.
