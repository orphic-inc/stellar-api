...
| **Frontend**                 | `orphic-inc/stellar-ui`              | stellar-ui `CONTEXT.md` _(pending — see note below)_                                        | stellar-ui `docs/adr/` _(pending)_ | React/TS client; the theming subsystem glossary                               |
...
> stellar-ui's `CONTEXT.md` currently lands via the `wip/theming-corpus` branch (PR pending); the link resolves once merged. stellar-ui has no `docs/adr/` home yet either — tracked on the UI tracker; both cells above are marked _pending_ until they land so this map stays honest.
