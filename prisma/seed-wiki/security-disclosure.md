...
A minimal demonstration that the issue is real is welcome. A working exploit, or anything built to be run by someone else, is not — do not attach one, and do not link one.
...
Golden Rule 6.1 prohibits seeking or exploiting bugs on the live site. Confirming a suspected vulnerability by exploiting it is itself the breach, even where the motive was to write a better report. Verify against a local development environment instead, or simply report the suspicion and let staff verify it.
...
Please keep it to yourself until it is fixed, including after a fix is announced if staff ask for time to let installations update.
