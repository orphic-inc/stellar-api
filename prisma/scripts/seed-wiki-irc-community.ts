...
const AUTHOR_ID =
  adminIdArg !== -1 ? parseInt(process.argv[adminIdArg + 1], 10) : 1;
...
async function main() {
  console.log(
    `Seeding wiki with ${PAGES.length} IRC community pages (authorId=${AUTHOR_ID})...`
  );
  ...
  if (!author) {
    console.error(
      `No user found with id=${AUTHOR_ID}. Run the install flow first or pass --admin-id.`
    );
    process.exit(1);
  }
