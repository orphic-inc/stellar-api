/**
 * Unit tests for the asset orphan sweep (ADR-0026 Phase 2, #342). The reference
 * extraction is pure and tested directly; the sweep itself runs against a mocked
 * Prisma so the delete predicate — owned, past grace, unreferenced — is pinned
 * exactly, since the failure mode of getting it wrong is deleting a live asset.
 */
...
describe('collectReferencedHashes', () => {
  it('unions references across stylesheet sources and avatars', async () => {
    prismaMock.authorStylesheet.findMany.mockResolvedValue([
      { source: `body{background:url(/api/asset/${H1})}` },
      { source: `a{background:url(/api/asset/${H2})}` }
    ] as never);
    prismaMock.user.findMany.mockResolvedValue([
      { avatar: `/api/asset/${H2}` }, // also referenced by a sheet — deduped
      { avatar: `/api/asset/${H3}` }
    ] as never);

    const refs = await collectReferencedHashes();

    expect(refs).toEqual(new Set([H1, H2, H3]));
  });
});
...
describe('sweepOrphanedAssets', () => {
  ...
  it('only considers owned assets past the grace window', async () => {
    ...
    // Cutoff is roughly one grace window ago.
