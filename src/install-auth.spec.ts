...
describe('API auth/profile/user flows', () => {
  ...
  it('logs in successfully and returns a user plus auth cookie', async () => {
    ...
    prismaMock.user.update.mockResolvedValue(
      asUserMock({
        ...
        // The row carries the warning rows; the wire carries `warnedUntil`.
        warnings: []
      })
    );
  ...
  it('reads only password-reset tokens at /recovery/reset', async () => {
    prismaMock.accountRecovery.findFirst.mockResolvedValueOnce(null);

    const res = await request(app)
      .post('/api/auth/recovery/reset')
      .send({ token: 'some-other-token', newPassword: 'new-password-123' });
